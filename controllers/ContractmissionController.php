...
class ContractmissionController extends Ep_Controller_Action
{
	...
	function init()
	{
		...
		$this->_view->producttype_array=$this->producttype_array=array(
								...
								"news"=>"News",
								"autre"=>"Others");
		$this->seo_producttype_array=array(
	...
	function createContractAction()
	{
		...
		if($requestparmas['quote_id'])
		{
			...
			$existsignedval = $quote_obj->checkcontractexist($requestparmas['quote_id']);
					if($existsignedval[0]['quotecontractid']>0)
			...
			if($status[0]['sales_review']=="signed" || $status[0]['sales_review']=="closed")
			{
				...
				if($missionDetails)
				{
					...
					for($i=0;$i<count($missionDetails);$i++)
					{
						...
						if($missionDetails[$i]['product_type_name']=="")
						{
							$missionDetails[$i]['product_type_name']=$this->seo_producttype_array[$missionDetails[$i]['product_type']];	
						}
                ...
                $clientContacts = $clientcontact_obj->getClientContactsDetails($status[0]['client_id']);
                $this->_view->clientContacts = $clientContacts;
	...
	function getSplitMonthAction()
	{
		...
		{
			$configdate=$this->configval['configure_expected_date'];
			...
			else */
			{
				...
				$turnovers = array();
				$months = array();
				/* Turnover for tech missions before linked with all */
				$tech_obj = new Ep_Quote_TechMissions();
				$tech_missions = $tech_obj->getTechMissionDetails(array('quote_id'=>$quote_id,'include_final'=>'yes','prod_linked'=>'all_prod','before_prod'=>'Before' ));
				if($tech_missions)
				{
					$i = 0;
					for($i=0;$i<count($tech_missions);$i++)
					{
						...
						if($tech_missions[$i]['oneshot']=='yes')
						{
							$turnover = $tech_missions[$i]['turnover'];
							if($tech_missions[$i]['tempo']=="fix")
								{
									$turnover = ($turnover/$tech_missions[$i]['volume'])*$tech_missions[$i]['volume_max'];
									$turnover = $this->getFinalTurnover($turnover,$tech_missions[$i]['tempo_length_option'],$tech_missions[$i]['tempo_length']);
								}						
						}
						else
						{
				...
				if($missionDetails)
				{
					...
					for($i=0;$i<count($missionDetails);$i++)
					{
						$oneshot = true;
						//echo $date1;
						if( count($techdays)>0 && $missionDetails[$i]['product']!='content_strategy')
						{
							$maxy=max($techdays);
							$date1=date('Y-m-d', strtotime($orgdate1."+ $maxy days"));
						}
						else
						$date1 = $orgdate1;
						 /*get linked mission before mission starts*/		
						 $linked_details=$this->getlinkedprod($quote_id,$missionDetails[$i]['identifier'],'Before');
						 $beforelinked_days=0;
						 if(count($linked_details))
						 {
							foreach ($linked_details as $linkedloop) 
							{
								$beforelinked_days+=$linkedloop['delivery_option']=="days"?$linkedloop['delivery_time']:ceil($linkedloop['delivery_time']/24);
							}
						  }
						/*considering contract mission turnover*/
						if($missionDetails[$i]['cm_turnover']!='0.00' && $missionDetails[$i]['cm_turnover'] )
							$turnover = $missionDetails[$i]['cm_turnover'];
						else	
						...
						/* For seo missions */
						if($missionDetails[$i]['product']=='seo_audit' || $missionDetails[$i]['product']=='smo_audit' || $missionDetails[$i]['product']=='content_strategy')
						{
							$type = $this->product_array[$missionDetails[$i]['product']];
							if($missionDetails[$i]['product']=='content_strategy')
							{
								$product_type = $this->product_array[$missionDetails[$i]['product']];
							}
							else
							{
						...
						else
						{
							...
							if($missionDetails[$i]['oneshot']=='yes')
							{
								$staff_days = $missionDetails[$i]['staff_time_option']=="days"?$missionDetails[$i]['staff_time']:ceil($missionDetails[$i]['staff_time']/24);
									if(date('d/m/Y',strtotime($configdate)) <= date("d/m/Y",strtotime($date2)))
									{
										$days = $days;	
									}
									else
						...
						/*getting before prod link details*/
						//echo $beforelinked_days.' '.$date1;
						if($beforelinked_days>0)
						{
							$date1 = date('Y-m-d',strtotime($date1."+ $beforelinked_days days"));	
						$date2 = date('Y-m-d',strtotime($date1."+ $days days"));
						}
						else
						{
						...
						/*after link with prod calculation*/
						$linkedafterdetails=$this->getlinkedprod($quote_id,$missionDetails[$i]['identifier'],'After');
						$afterlinked_days=0;
						 if(count($linkedafterdetails) && $missionDetails[$i]['product']!='content_strategy')
						 {
							foreach ($linkedafterdetails as $afterloop) 
							{
								$afterlinked_days+=$afterloop['delivery_option']=="days"?$afterloop['delivery_time']:ceil($afterloop['delivery_time']/24);
								$afterlinked[$afterloop['identifier']]['prod']=$days;
								$afterlinked[$afterloop['identifier']]['staff']=$staff_days;
								$afterlinked[$afterloop['identifier']]['before']=$beforelinked_days;
							}
						  }
					}
					//echo $afterlinked_days; 
				}	

				//	echo '<pre>'; print_r($afterlinked); 

				/* Turnover for tech missions same as linked with one prod */
				$tech_missions = $tech_obj->getTechMissionDetails(array('quote_id'=>$quote_id,'include_final'=>'yes','all_linked'=>'all_prod'));
				if($tech_missions)
				{
					$i = 0;
					for($i=0;$i<count($tech_missions);$i++)
					{
						...
						if($tech_missions[$i]['package']=='team')
							$turnover += $tech_missions[$i]['team_fee'] * $tech_missions[$i]['team_packs'];
						/*after prod mission start*/
						if( count($afterlinked[$tech_missions[$i]['identifier']])>0 && $tech_missions[$i]['before_prod']=='After' && $tech_missions[$i]['prod_linked']!='all_prod')
						{
							$misdays=$afterlinked[$tech_missions[$i]['identifier']]['prod'];
							$staff_days=$afterlinked[$tech_missions[$i]['identifier']]['staff'];
							$before_days=$afterlinked[$tech_missions[$i]['identifier']]['before'];
							$techstartdays=$staff_days+$misdays+$before_days;
							$date1 = date('Y-m-d',strtotime($date1."+ $techstartdays days"));
							//echo $date1;  exit;	
						}
						else
						{
				...
				//linked with prod after all prod
				$tech_missions = $tech_obj->getTechMissionDetails(array('quote_id'=>$quote_id,'include_final'=>'yes','prod_linked'=>'all_prod','before_prod_all'=>'Before'));
				if($tech_missions)
				{
					$i = 0;
					for($i=0;$i<count($tech_missions);$i++)
					{
						if(count($alldays)>0){
						$max=max($alldays);
						$date1= date('Y-m-d',strtotime($orgdate1."+ $max days"));
						}
						else
							$date1 = $orgdate1;
						//echo $date1.'exit'; exit;
			...
			$total = $cturnover = array();
			/* Sorting and formatting to display */
			foreach($turnovers as $mid=>$mission_id)
			{
				foreach($mission_id as $type=>$article_type)
				{
					foreach($article_type as $month => $value)
					{
						$total[$month] += $value;
					}
					foreach($months as $monthname)
					{						
						if(!array_key_exists($monthname,$article_type)){
							$cturnover[$mid][$type][$monthname]='-';
						}	
						else
							$cturnover[$mid][$type][$monthname] =$article_type[$monthname];
					}
				}
			}
			ksort($total);
			$this->_view->turnovers = $cturnover;
			$this->_view->total_per_month = $total;
	...
	function saveContractAction()
	{
        ...
        {  
			...
			$save['signed_PO'] = ( $params['signed_PO'] == 'on' ) ? 'yes' : 'no' ;//signed_PO 'yes' or 'no'
			$save['contact_client'] =$params['contact_client'];
...
//				echo "<pre>";print_r($save);exit;
				...
				{
					$users_list[]=$bo_user_details[0]['email'];
				}
				
				$facturationUsers = $quote_contract->getUsers("facturation");
				foreach($facturationUsers as $row)
				{
					$users_list[]=$row['email'];
				}
                        
						/* *** added on 22.01.2016 **** */
                        //this functin is used to send the email after the contract is created//
                        $email = array();
                        $email['title'] = $params['contract_name'];
						$email['contract_turnover'] = $params['turnover'];
						$email['currency'] = $quote_res[0]['sales_suggested_currency'];						 
						$email['client_id'] = $client_id;
						$email['company_name'] = $company_name;
                        $email['link'] = BO_DOMAIN_.'contractmission/contract-edit?submenuId=ML13-SL3&contract_id='.$contractid.'&action=view';
						$email['download_po']=BO_DOMAIN_.'followup/download-po?cid='.$contractid;

						$prodMissionDetails= $this->getMissionDetails($params['quote_id']);	
						...
						$tech_obj=new Ep_Quote_TechMissions();
						$techMissionDetails=$tech_obj->getTechMissionDetails(array("quote_id"=>$params['quote_id']));
						if($techMissionDetails)
						{
						$email['techMissiondetails']=$techMissionDetails;
							//echo "<pre>";print_r($techMissionDetails);exit;
							/*calculating Tech turnover*/
							$tech_turnover=0;
							foreach($techMissionDetails as $tmission)
							{								
								$tturnover=$tmission['free_mission']=='yes'? 0 : $tmission['turnover'];
						...
						$seoParameters['quote_id']=$params['quote_id'];
						$seoParameters['misson_user_type']='seo';
						$seoParameters['include_final']='yes';
						$seoParameters['product_type_seo']='IN';
						$seoMissionDetails = $seomission_obj->getMissionDetails($seoParameters);
			...
			foreach($params['invoice_per'] as $key => $value)
			{
				...
				$qmsave['history_bo'] = $params['history_bo'][$key]=='yes' ? 'yes' : 'no';
				$qmsave['history_fo'] = $params['history_fo'][$key]=='yes' ? 'yes' : 'no';				
				
				if($params['missiontable'][$key]=='qm')
				{
					//echo "<pre>";print_r($qmsave);exit;
            ...
            $documents = $this->uploadFiles($_FILES,$contractid,$quote_contract,$params['document_name']);
    ...
    function getMissionDetails($quote_id)
    {
        if ($quote_id) {
            $quote_obj = new Ep_Quote_Quotes();
            $status = $quote_obj->getQuoteDetails($quote_id);

            $existsignedval = $quote_obj->checkcontractexist($quote_id);
            //if ($existsignedval[0]['quotecontractid'] > 0)
                //$this->_redirect("/quote/sales-quotes-list?submenuId=ML13-SL2");
	...
	function uploadFiles($FILES,$contractid,$quote_contract,$pdocument_name)
	{
		...
		if(count($FILES['mulitupload']['name'])>0)	
		{
            ...
            return $documents;//retruning the path names of all the files uploaded(for attachment purpose to mail)//
		}
				...
				if($contract[0]['contractfilepaths'])
				{
					...
					foreach($documents_path as $k=>$file)
					{
						if(file_exists($this->contract_documents_path.$documents_path[$k]) && !is_dir($this->contract_documents_path.$documents_path[$k]))
						{
							...
							$related_files.='
							<div class="topset2"><a href="/contractmission/download-document?type=contract&index='.$k.'&contract_id='.$requestparmas['contract_id'].'">'.$file_name.'</a><span class="delete" rel="'.$k.'_'.$requestparmas['contract_id'].'" id="'.$k.'_'.$requestparmas['contract_id'].'"> <i class="splashy-error_x"></i></span></div>';
						}
					}
				}
	...
	function contractEditAction()
	{
		...
		if($requestparmas['contract_id'])
		{
			...
			if($contract)
			{
			    ...
			    /* added on 25.01.2016 */
                $clientcontact_obj = new Ep_Quote_ClientContacts();
                $clientContacts = $clientcontact_obj->getClientContactsDetails($quoteDetials[0]['client_id']);
                $this->_view->clientContacts = $clientContacts;
                //$this->render('edit-create-quote-contract-sales');
				$this->render('edit-contract');
			}
			...
			if($contract_details)
			{
				...
				elseif($params['type']=='seo' && ($this->_view->user_type=='seomanager' || $this->_view->user_type=='superadmin'))
				{
					...
					if(($index < $this->_view->total_seo_mission_count) || $params['cmid'])
					{
						...
						$this->_view->users = $quote_contract->getUsers("'seouser' OR type='seomanager'",true,true);
			...
			$params['writing'] = str_replace(",",".",$params['writing']);
				if($params['contractmissionid']):
					...
					if($params['missiontype']=='prod')
					{
						...
						/* *** added on 24.02.2016 related to BNP dev*** */

                        if($params['bnp_mission'])
                            $save['bnp_mission'] = $params['bnp_mission'];
                        /* *** end of added on 24.02.2016 related to BNP dev*** */
					...
					$save['history_bo'] = $qmdetails[0]['history_bo'];
					$save['history_fo'] = $qmdetails[0]['history_fo'];
					$save['unit_price'] = $qmdetails[0]['unit_price'];
					$save['volume'] = $qmdetails[0]['volume'];
					$save['currency'] = $qmdetails[0]['sales_suggested_currency'];
					if($qmdetails[0]['turnover'])
						$save['cm_turnover'] = $qmdetails[0]['turnover'];
					else
						$save['cm_turnover'] = "0";
				...
				if($params['missiontype']=='tech' && $params['update_tech']==1)
				{
					...
					if($params['free_mission'])
					{
						$tech_update['free_mission'] = $params['free_mission'];
						if($tech_update['free_mission']!='yes')
						{
							$unit_price =number_format(($params['tech_cost']/(1-(60/100))),2, '.', '');
						    $tech_update['turnover'] = $unit_price;
							$contract_update = array();
							$contract_update['turnover'] = $contract[0]['turnover']+$tech_update['turnover'];
							$contract_update['old_turnover'] = $contract[0]['turnover'];
							$quote_contract->updateContract($contract_update,$params['contractid']);
							if($tech_update['delivery_option']!='days')
								$days = ceil($tech_update['delivery_time']/24);
							else
							{
								$days = $tech_update['delivery_time'];
							}
							$launch_date = $contract[0]['expected_launch_date'];
							$splitvalues = $this->getTechSplitValues($tech_update['turnover'],$days,$launch_date);
							if(count($splitvalues)):
								$insert_sp = array();
								$insert_sp['contract_id'] = $contract[0]['quotecontractid'];
								$insert_sp['mission_id'] = $params['seotechprodid'];
								$insert_sp['quote_id'] = $contract[0]['quoteid'];
								$insert_sp['name'] = $tech_update['title'];
								$quote_contract->deleteSplitTurnovers($contract[0]['quotecontractid'],$params['seotechprodid']);
								foreach($splitvalues['turnover'] as $key => $value)
								{
									$insert_sp['month_year'] = $key;
									$insert_sp['turnover'] = $value;
									$quote_contract->insertSplitTurnovers($insert_sp);								
								}
							endif;
						}
					}
	...
	function getSeoTechFiles($quote_id)//use delete parameter later
	{
		$files = "";
	...
	function missionsListAction()
	{
		...
		elseif($this->_view->user_type=='superadmin')
		{
			...
			//echo "<pre>";print_r($sales_seo_missions);exit;
			...
			foreach($sales_seo_missions as $row)
			{
				...
				if($row['misson_user_type']=='sales' || ($row['misson_user_type']=='seo' && $row['product']!='seo_audit' && $row['product']!='smo_audit' && $row['product']!='content_strategy'))
				{
					$row['type'] = 'prod';
					$row['index'] = $prodIndex++;
				}
					...
					if(!empty($pmid))
					{
						if($pmid==$row['assigned_to'])
						{
							if($row['cm_status']=='ongoing')
							{
								...
								$prod_seo_tech['opened'][$row['client_id']]['contract_list'][$row['quotecontractid']]['mission_list'][] = $formatted_row;
								$prod_seo_tech['opened'][$row['client_id']]['company_name']=$row['company_name'];
								$prod_seo_tech['opened'][$row['client_id']][$row['quotecontractid']]['contractname']=$row['contractname'];
							}	
							elseif($row['cm_status']=='deleted')
							{
								...
								$prod_seo_tech['deleted'][$row['client_id']]['contract_list'][$row['quotecontractid']]['mission_list'][] = $formatted_row;
								$prod_seo_tech['deleted'][$row['client_id']]['company_name']=$row['company_name'];
								$prod_seo_tech['deleted'][$row['client_id']][$row['quotecontractid']]['contractname']=$row['contractname'];
							}
							else
							{
								if($row['assigned_to'])
								...
								$prod_seo_tech['finished'][$row['client_id']]['contract_list'][$row['quotecontractid']]['mission_list'][] = $formatted_row;
								$prod_seo_tech['finished'][$row['client_id']]['company_name']=$row['company_name'];
								$prod_seo_tech['finished'][$row['client_id']][$row['quotecontractid']]['contractname']=$row['contractname'];
							}
						}						
					}
					else
				...
				else
				{
					...
					$prod_seo_tech['to_assign'][$row['client_id']]['contract_list'][$row['quotecontractid']]['mission_list'][] = $formatted_row;
					$prod_seo_tech['to_assign'][$row['client_id']]['company_name']=$row['company_name'];
					$prod_seo_tech['to_assign'][$row['client_id']][$row['quotecontractid']]['contractname']=$row['contractname'];
			...
			$contracts = $con_mis_obj->getContracts(array('mulitple_status'=>"'validated','closed','deleted'",'cid'=>$search['contract_id'],'client_id'=>$search['client_id']));
			foreach($contracts as $row)
			{
				$searchParameters = array();
				...
				foreach($techMissionDetails as $tech_row)
				{
					...
					$tech_row['client_id'] = $row['clientid'];
						...
						$prod_seo_tech['to_assign'][$row['client_id']]['contract_list'][$row['quotecontractid']]['mission_list'][] = $this->formatTechMission($tech_row,$row);
						$prod_seo_tech['to_assign'][$row['client_id']]['company_name']=$row['company_name'];
						$prod_seo_tech['to_assign'][$row['client_id']][$row['quotecontractid']]['contractname']=$row['contractname'];
					}
				}
			...
			$staffing_missions = $con_mis_obj->getStaffMissions(array('contract_id'=>$contract_id,'client_id'=>$client_id));
				...
				foreach($staffing_missions as $row)
				{
					...
					if($row['cmid'])
					{
						...
						if($row['cm_status']=='ongoing')
						{
							if($row['assigned_to'])
							$prod_seo_tech['assigned_users'][$row['assigned_to']] = $row['pm'];
							$prod_seo_tech['opened'][$row['client_id']]['contract_list'][$row['contract_id']]['mission_list'][] = $formatted_row;
							$prod_seo_tech['opened'][$row['client_id']]['company_name']=$row['company_name'];
							$prod_seo_tech['opened'][$row['client_id']][$row['contract_id']]['contractname']=$row['contractname'];
						}
						elseif($row['cm_status']=='deleted')
						{
							//$prod_seo_tech['deleted'][$row['contract_id']][] = $formatted_row;
							$prod_seo_tech['deleted'][$row['client_id']]['contract_list'][$row['contract_id']]['mission_list'][] = $formatted_row;
							$prod_seo_tech['deleted'][$row['client_id']]['company_name']=$row['company_name'];
							$prod_seo_tech['deleted'][$row['client_id']][$row['contract_id']]['contractname']=$row['contractname'];
							if($row['assigned_to'])
								$prod_seo_tech['deleted_users'][$row['assigned_to']] = $row['pm'];
						}
						else
						{
							...
							$prod_seo_tech['finished'][$row['client_id']]['contract_list'][$row['contract_id']]['mission_list'][] = $formatted_row;
							$prod_seo_tech['finished'][$row['client_id']]['company_name']=$row['company_name'];
							$prod_seo_tech['finished'][$row['client_id']][$row['contract_id']]['contractname']=$row['contractname'];
							if($row['assigned_to'])
								$prod_seo_tech['finished_users'][$row['assigned_to']] = $row['pm'];
						}
					}
					else
					{
						...
						$prod_seo_tech['to_assign'][$row['client_id']]['contract_list'][$row['contract_id']]['mission_list'][] = $formatted_row;
						$prod_seo_tech['to_assign'][$row['client_id']]['company_name']=$row['company_name'];
						$prod_seo_tech['to_assign'][$row['client_id']][$row['contract_id']]['contractname']=$row['contractname'];
					}
					$prev_contract_id = $row['contract_id'];
				}
			}
			/* getting sales mission */
			$sales_mission = $con_mis_obj->getSalesMissions(array('contract_id'=>$contract_id,'type'=>'sales','client_id'=>$client_id));
		...
		//echo "<pre>";print_r($prod_seo_tech);exit;
		/* if($this->_view->user_type=='superadmin')
		...
		foreach($prod_seo_tech['deleted_users'] as $keydel=>$delval)
		{
			$alluserlist[$keydel]=$delval;
		}
		foreach ($prod_seo_tech['assigned_users'] as $keyass => $valueas) {
			$alluserlist[$keyass]=$valueas;
		}
		foreach ($prod_seo_tech['finished_users'] as $keyfini => $valuefini) {
			$alluserlist[$keyfini]=$valuefini;
		}
		natcasesort($alluserlist);
		$this->_view->alluserlist= $alluserlist;
		...
		*/
		$contractclientloop=$con_mis_obj->getContracts(array('mulitple_status'=>"'validated','closed','deleted'",'client_id'=>$client_id));
		foreach($contractclientloop as $clientcontractdetails)
		{
			$contractclientlist[$clientcontractdetails['quotecontractid']]=$clientcontractdetails['contractname'];
		}
		natsort($contractclientlist);
		$this->_view->contractlist = $contractclientlist;
		$client_contract=$con_mis_obj->getContracts(array('mulitple_status'=>"'validated','closed','deleted'"));
		foreach($client_contract as $clientdetails)
		{
			$clientlist[$clientdetails['clientid']]=$clientdetails['company_name'];
		}
		natsort($clientlist);
		...
		foreach($prod_seo_tech['ongoing_languages'] as $keyon=>$delon)
		{
			$alllanguagelist[$keyon]=$delon;
		}
		foreach ($prod_seo_tech['deleted_languages'] as $keydel => $valuedel) {
			$alllanguagelist[$keydel]=$valuedel;
		}
		foreach ($prod_seo_tech['finished_languages'] as $keyfini => $valuefini) {
			$alllanguagelist[$keyfini]=$valuefini;
		}
		foreach ($prod_seo_tech['toassign_languages'] as $keyas => $valueas) {
			$alllanguagelist[$keyas]=$valueas;
		}
		natcasesort($alllanguagelist); 
		$this->_view->alllanguagelist= $alllanguagelist;
			...
			foreach($contracts as $contract => $quote_id)
			{
				...
				foreach($prod_seo_missions as $prod_seo)
				{
					...
					else
					{
						...
						$insert['cm_status'] = $request['bulkstatus'];
						$insert['contract_id'] = $contract;
						if($prod_seo['misson_user_type']=='sales' || ($prod_seo['misson_user_type']=='seo' && $prod_seo['product']!='seo_audit' && $prod_seo['product']!='smo_audit' && $prod_seo['product']!='content_strategy'))
						{
							$insert['type'] = 'prod';
						}
						else
	...
	function missionslistUsers($search=array(),$lang="")
	{
		...
		if($this->_view->user_type=='techuser')
		{
			...
			foreach($assignedMissions as $row)
			{
				...
				if($row['cm_status']=='ongoing')
				{
					$prod_seo_tech['opened'][$row['client_id']]['contract_list'][$row['quotecontractid']]['mission_list'][] = $this->formatTechMission($row,$row);;
					$prod_seo_tech['opened'][$row['client_id']]['company_name']=$row['company_name'];
					$prod_seo_tech['opened'][$row['client_id']][$row['quotecontractid']]['contractname']=$row['contractname'];
				}
				elseif($row['cm_status']=='deleted')
				...
				$prod_seo_tech['deleted'][$row['client_id']]['contract_list'][$row['quotecontractid']]['mission_list'][] = $this->formatTechMission($row,$row);
				$prod_seo_tech['deleted'][$row['client_id']]['company_name']=$row['company_name'];
				$prod_seo_tech['deleted'][$row['client_id']][$row['quotecontractid']]['contractname']=$row['contractname'];
				}
				else
				...
				$prod_seo_tech['finished'][$row['client_id']]['contract_list'][$row['quotecontractid']]['mission_list'][] = $this->formatTechMission($row,$row);
				$prod_seo_tech['finished'][$row['client_id']]['company_name']=$row['company_name'];
				$prod_seo_tech['finished'][$row['client_id']][$row['quotecontractid']]['contractname']=$row['contractname'];
				}
			}
			...
			if($staffing_missions)
			{
				...
				foreach($staffing_missions as $row)
				{
					...
					if($row['cmid'])
					{
						...
						$prod_seo_tech['opened'][$row['client_id']]['contract_list'][$row['contract_id']]['mission_list'][] = $this->formatStaffMission($row);
						$prod_seo_tech['opened'][$row['client_id']]['company_name']=$row['company_name'];
						$prod_seo_tech['opened'][$row['client_id']][$row['contract_id']]['contractname']=$row['contractname'];
						}
						if($row['cm_status']=='deleted')
						{
							...
							$prod_seo_tech['deleted'][$row['client_id']]['contract_list'][$row['contract_id']]['mission_list'][] = $this->formatStaffMission($row);
							$prod_seo_tech['deleted'][$row['client_id']]['company_name']=$row['company_name'];
							$prod_seo_tech['deleted'][$row['client_id']][$row['contract_id']]['contractname']=$row['contractname'];
						}
						else
						{
							...
							$prod_seo_tech['finished'][$row['client_id']]['contract_list'][$row['contract_id']]['mission_list'][] =$this->formatStaffMission($row);
							$prod_seo_tech['finished'][$row['client_id']]['company_name']=$row['company_name'];
							$prod_seo_tech['finished'][$row['client_id']][$row['contract_id']]['contractname']=$row['contractname'];
						}
					}
					else
					{
						...
						$prod_seo_tech['to_assign'][$row['client_id']]['contract_list'][$row['contract_id']]['mission_list'][] = $this->formatStaffMission($row);
						$prod_seo_tech['to_assign'][$row['client_id']]['company_name']=$row['company_name'];
						$prod_seo_tech['to_assign'][$row['client_id']][$row['contract_id']]['contractname']=$row['contractname'];
					}
					$prev_contract_id = $row['contract_id'];
		...
		else if($this->_view->user_type=='facturation')
		{
			...
			foreach($sales_mission as $row)
			{
				...
				$prod_seo_tech['opened'][$row['client_id']]['contract_list'][$row['contract_id']]['mission_list'][] = $this->formatSalesMission($row);
				$prod_seo_tech['opened'][$row['client_id']]['company_name']=$row['company_name'];
				$prod_seo_tech['opened'][$row['client_id']][$row['contract_id']]['contractname']=$row['contractname'];	
				}
				elseif($row['cm_status']=='deleted')
				...
				$prod_seo_tech['deleted'][$row['client_id']]['contract_list'][$row['contract_id']]['mission_list'][] = $this->formatSalesMission($row);
				$prod_seo_tech['deleted'][$row['client_id']]['company_name']=$row['company_name'];
				$prod_seo_tech['deleted'][$row['client_id']][$row['contract_id']]['contractname']=$row['contractname'];
				}
				else
				...
				//$prod_seo_tech['finished'][$row['contract_id']][] = $this->formatSalesMission($row);
				  $prod_seo_tech['finished'][$row['client_id']]['contract_list'][$row['contract_id']]['mission_list'][] =$this->formatSalesMission($row);
					$prod_seo_tech['finished'][$row['client_id']]['company_name']=$row['company_name'];
					$prod_seo_tech['finished'][$row['client_id']][$row['contract_id']]['contractname']=$row['contractname'];
				}
			}
	...
	function missionslistManagers($search=array(),$lang="")
	{
					...
					if($explode[4]=='ongoing')
						{
							...
							$prod_seo_tech['opened'][$row['client_id']]['contract_list'][$row['quotecontractid']]['mission_list'][] = $this->formatTechMission($tech_row,$row);
							$prod_seo_tech['opened'][$row['client_id']]['company_name']=$row['company_name'];
							$prod_seo_tech['opened'][$row['client_id']][$row['quotecontractid']]['contractname']=$row['contractname'];
						}
						elseif($explode[4]=='deleted')
						{
							...
							$prod_seo_tech['deleted'][$row['client_id']]['contract_list'][$row['quotecontractid']]['mission_list'][] = $this->formatTechMission($tech_row,$row);
							$prod_seo_tech['deleted'][$row['client_id']]['company_name']=$row['company_name'];
							$prod_seo_tech['deleted'][$row['client_id']][$row['quotecontractid']]['contractname']=$row['contractname'];
						}
					else
						{
							...
							$prod_seo_tech['finished'][$row['client_id']]['contract_list'][$row['quotecontractid']]['mission_list'][] =$this->formatTechMission($tech_row,$row);
							$prod_seo_tech['finished'][$row['client_id']]['company_name']=$row['company_name'];
							$prod_seo_tech['finished'][$row['client_id']][$row['quotecontractid']]['contractname']=$row['contractname'];
						}
					}
		...
		else
		{
			...
			$search['mission_type'] = " AND qm.product IN('seo_audit','smo_audit','content_strategy')";
			elseif($this->_view->user_type=='prodmanager')
			$search['mission_type'] = " AND qm.product NOT IN('seo_audit','smo_audit','content_strategy')";
	...
	function formatMission($row)
	{
		...
		$array['nb_words'] = $row['nb_words']!='' ? $row['nb_words'] : "-";
		return $array;
	}
	...
	function formatTechMission($tech_row,$row)
	{
		...
		$array['nb_words'] = $tech_row['nb_words']!='' ? $tech_row['nb_words'] : "-";
		return $array;
	}
	...
	function formatStaffMission($staff_row)
	{
		...
		$array['nb_words'] = $staff_row['nb_words']!='' ? $staff_row['nb_words'] : "-";
		return $array;
	}
	...
	function formatSalesMission($sales)
	{
		...
		$array['nb_words'] = $sales['nb_words']!='' ? $sales['nb_words'] : "-";
		return $array;
	}
	...
	function loadmissionsAction()
	{
		if($this->_request->isPost() && $_SERVER['HTTP_X_REQUESTED_WITH']=='XMLHttpRequest')
		{
			...
			//if($request['opened'])
			{
				...
				$search['client_id'] = $request['client_id'];
				...
				elseif($this->_view->user_type=='superadmin')
				{
					...
					/* get staffing missions */
					$staffing_missions = $con_mis_obj->getStaffMissions(array('contract_id'=>$request['cid'],'assigned_to'=>$request['pmid'],'client_id'=>$request['client_id']));
					if($staffing_missions)
					{
						$staffindex = 0;
						$prev_contract_id = "";
					...
					/* getting sales mission */
					$sales_mission = $con_mis_obj->getSalesMissions(array('contract_id'=>$request['cid'],'type'=>'sales','assigned_to'=>$request['pmid'],'client_id'=>$request['client_id']));
					foreach($sales_mission as $row)
					{
						if($row['cm_status']=='ongoing')
						{
				...
				if($request['opened']==3)
				{
					...
					$contractclient=$con_mis_obj->getContracts(array('mulitple_status'=>"'validated','closed','deleted'",'client_id'=>$request['client_id']));
					foreach($contractclient as $clientcontractdetails)
					{
						$contractclientlist[$clientcontractdetails['quotecontractid']]=$clientcontractdetails['contractname'];
				}
					natsort($contractclientlist); 
	...
	function deleteDocumentStaffAction()
	{
		if($this->_request->isPost() && $_SERVER['HTTP_X_REQUESTED_WITH']=='XMLHttpRequest')
		{
			...
			$identifier = $explode_identifier[1];
			$contract_obj=new Ep_Quote_Quotecontract();
            $result = $contract_obj->getStaffMissionDetails(array('staff_missionId'=>$identifier));
    ...
    /* Import Quote and Quotemissions through xls */
        ...
        {
            ini_set('max_execution_time', 300);
            $document_name = "FI_Quote-Quotemissions-Weather_stencils.xlsx";
	    $document_path= $_SERVER['DOCUMENT_ROOT'].'/BO/oldmissionsupload/'.$document_name;
	...
	function getlinkedprod($quote_id,$mission_id,$before)
	{
		$quotecontract_obj=new Ep_Quote_Quotecontract();

		$linkedmission_details=$quotecontract_obj->techmissionlinked($quote_id,$mission_id,$before);

		return $linkedmission_details;
	}
}

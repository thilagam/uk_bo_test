...
class BnpQuotedeliveryController extends ContractmissionController
{		
    ...
    public function autoDeliverySession($delivery_id,$mission_id)
    {
    	if($delivery_id)
    	{
    		...
    		{
				$this->_redirect("/bnp-quotedelivery/delivery-prod1?mission_id=$mission_id&daction=new&submenuId=ML13-SL4");
    		}
		...
		if($mission_id)
		{
			...
			{	
				...
				foreach($misssionQuoteDetails as $pkey=>$deliveryMission)
				{
                    ...
                    $CityArticleCounts = (array) $bnp_obj->getCityrewriteArticleCount();
                    $sCityArticleCount = array();
                    foreach($CityArticleCounts as $CityArticleCount)
                    {
                        $sCityArticleCounts[$CityArticleCount['city_id']] = $CityArticleCount['rewrite_article_count_fr'];
                    }
                    $this->_view->sCityArticleCounts = $sCityArticleCounts;

                    $sample_texts = (array) $bnp_obj->getSampleTextIds();
                    $ssample_texts = array();
                    foreach($sample_texts as $sample_text)
                    {
                        $ssample_texts[$sample_text['city_id']] = $sample_text['sample_id'];
                    }
                    $this->_view->sample_text=$ssample_texts;
				...
				$this->render('bnp-delivery-prod1');
			}
			else
		...
		if($this->_request->isPost())
		{
			...
			if($mission_id)
			{
                ...
                if(!empty($step1Params['city']))
                {
                    ...
                    $this->delivery_creation->prod_step1['sample_text_id'] = $step1Params['sample_text_id'];
                    $this->delivery_creation->prod_step1['no_of_articles'] = $step1Params['no_of_articles'];
                }
				...
				$this->_redirect("/bnp-quotedelivery/delivery-prod2?mission_id=".$mission_id);
			}				
		}	
		...
		if($this->delivery_creation->prod_step1['total_article']>0 && $mission_id)
		{
			...
			if($this->delivery_creation->prod_step1['product']=='translation' && $this->delivery_creation->prod_step1['mandatory_token'])
			{
				...
				if($translateStencils)
				{
					...
					{
						$this->_redirect('/bnp-quotedelivery/delivery-prod1?mission_id='.$mission_id);
					}
				}
			...
			$this->render('bnp-delivery-prod2');
		}
		else
			$this->_redirect("/bnp-quotedelivery/delivery-prod1?mission_id=".$mission_id);
	}
	...
	public function deliveryProd3Action()
	{
		...
		if($this->delivery_creation->prod_step1['total_article']>0 && $mission_id)
		{
			...
			$this->render('bnp-delivery-prod3');
		}		
		else
			$this->_redirect("/bnp-quotedelivery/delivery-prod1?mission_id=".$mission_id);
	}
			...
			if($this->delivery_creation->prod_step1['total_article']>0 && $delivery_identifier)
			{
				...
				for($i=1;$i<=$total_article;$i++)
				{
                    ...
                    if($this->delivery_creation->prod_step1['city']){
                        $article_array["bnp_city_id"] = implode('|',$this->delivery_creation->prod_step1['city']);
                        $article_array["bnp_sampletext_id"] = implode('|',$this->delivery_creation->prod_step1['sample_text_id']);
                        $article_array["bnp_no_of_articles"] = implode('|',$this->delivery_creation->prod_step1['no_of_articles']);
                    }
